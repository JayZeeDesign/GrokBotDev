...
export const GET: APIRoute = async () => {
  ...
  const items = all.map(toFeedItem);
  return jsonResponse(
    envelope(items, {
      ...
      newest_added_at: items.length ? all[0].data.added_at : null,
      note: 'Lean feed. Fetch an item.detail_url for its full record incl. the prompt.',
    })
  );
};
